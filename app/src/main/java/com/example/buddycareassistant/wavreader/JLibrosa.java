...
/**
 *
 * This Class is an equivalent of Python Librosa utility used to extract the
 * Audio features from given Wav file.
 *
 * @author abhi-rawat1
 *
 */
public class JLibrosa {
    ...
    /* yangqin: get log mel spectrogram */
    public double[][] getLogMelSpec(float[] y) {
        AudioFeatureExtraction getlogmelspe = new AudioFeatureExtraction();
        double[][] logmelspec = getlogmelspe.logmelspec(y);
        return logmelspec;
    }
    ...
    private float[][] readMagnitudeValuesFromFile(String path, int sampleRate, int readDurationInSeconds,
                                                  int offsetDuration) throws IOException, WavFileException, FileFormatNotSupportedException {
     ...
     * @return
     */
    public float[][] generateMFCCFeatures(float[] magValues, int mSampleRate, int nMFCC, int n_fft, int n_mels,
                                          int hop_length) {
    ...
    public float[][] generateMFCCFeatures(float[] magValues, int mSampleRate, int nMFCC) {
        ...
        float[][] mfccValues = this.generateMFCCFeatures(magValues, mSampleRate, nMFCC, this.n_fft, this.n_mels,
                this.hop_length);
    ...
    public float[] generateMeanMFCCFeatures(float[][] mfccValues, int nMFCC, int nFFT) {
        ...
        /*
         * for (int p = 0; p < nMFCC; p++) { double fftValAcrossRow = 0; for (int q = 0;
         * q < nFFT; q++) { fftValAcrossRow = fftValAcrossRow + mfccValues[p][q]; }
         * double fftMeanValAcrossRow = fftValAcrossRow / nFFT; meanMFCCValues[p] =
         * (float) fftMeanValAcrossRow; }
         */
    ...
    /**
     * This function calculates and returns the me of given Audio Sample values.
     * STFT stands for Short Term Fourier Transform
     *
     * @param magValues
     * @param sampleRate
     * @param nMFCC
     ...
     * @return
     */
    public Complex[][] generateSTFTFeatures(float[] magValues, int mSampleRate, int nMFCC, int n_fft, int n_mels,
                                            int hop_length) {
        Complex[][] stftValues = this.generateSTFTFeaturesWithPadOption(magValues, mSampleRate, nMFCC, n_fft, n_mels,
                hop_length, true);
        return stftValues;
    }
    ...
    /**
     * This function calculates and returns the STFT values of given Audio Sample
     * values with/without applying padding as one of the argument flag. STFT stands
     ...
     * @return
     */
    public Complex[][] generateSTFTFeaturesWithPadOption(float[] magValues, int mSampleRate, int nMFCC, int n_fft,
                                                         int n_mels, int hop_length, boolean paddingFlag) {
    ...
    /**
     * This function calculates and returns the inverse STFT values of given stft
     * values values. STFT stands for Short Term Fourier Transform
     *
     * @param magValues
     * @param nMFCC
     * @return
     */
    public float[] generateInvSTFTFeatures(Complex[][] stftValues, int mSampleRate, int nMFCC, int n_fft, int n_mels,
                                           int hop_length) {
        float[] magValues = this.generateInvSTFTFeaturesWithPadOption(stftValues, mSampleRate, nMFCC, n_fft, n_mels,
                hop_length, -1, false);
        return magValues;
    }
     ...
     */
    public float[] generateInvSTFTFeatures(Complex[][] stftValues, int mSampleRate, int nMFCC, int n_fft, int n_mels,
                                           int hop_length, int length) {
        float[] magValues = this.generateInvSTFTFeaturesWithPadOption(stftValues, mSampleRate, nMFCC, n_fft, n_mels,
                hop_length, length, false);
        return magValues;
    }

    /**
     * This function calculates and returns the inverse STFT values of given stft
     * values values. STFT stands for Short Term Fourier Transform This function to
     * be used for getting inverse STFT if STFT values have been generated with pad
     * values.
     *
     * @param magValues
     * @param nMFCC
     * @return
     */
    public float[] generateInvSTFTFeaturesWithPadOption(Complex[][] stftValues, int mSampleRate, int nMFCC, int n_fft,
                                                        int n_mels, int hop_length, int length, boolean paddingFlag) {
        AudioFeatureExtraction featureExtractor = new AudioFeatureExtraction();
        featureExtractor.setN_fft(n_fft);
        featureExtractor.setN_mels(n_mels);
        featureExtractor.setHop_length(hop_length);
    ...
    /**
     * This function calculates and returns the inverse STFT values of given STFT
     * Complex values. STFT stands for Short Term Fourier Transform
     *
     * @param magValues
     * @param nMFCC
     * @return
    ...
    public float[] generateInvSTFTFeatures(Complex[][] stftValues, int mSampleRate, int nMFCC) {
        ...
        float[] magValues = this.generateInvSTFTFeatures(stftValues, mSampleRate, nMFCC, this.n_fft, this.n_mels,
                this.hop_length);
        return magValues;
    }
    ...
    public Complex[][] generateSTFTFeatures(float[] magValues, int mSampleRate, int nMFCC) {
        ...
        Complex[][] stftValues = this.generateSTFTFeatures(magValues, mSampleRate, nMFCC, this.n_fft, this.n_mels,
                this.hop_length);
        return stftValues;
    }
     ...
     * @return
     */
    public Complex[][] generateSTFTFeaturesWithPadOption(float[] magValues, int mSampleRate, int nMFCC,
                                                         boolean padFlag) {

        Complex[][] stftValues = this.generateSTFTFeaturesWithPadOption(magValues, mSampleRate, nMFCC, this.n_fft,
                this.n_mels, this.hop_length, padFlag);
        return stftValues;
    }

    /**
     * This function loads the audio file, reads its Numeric Magnitude Feature
     * values and then takes the mean of amplitude values across all the channels
     * and convert the signal to mono mode by taking the average. This method reads
     * the audio file post the mentioned offset duration in seconds.
     *
     * @param path
     * @param sampleRate
     * @param readDurationInSeconds
    ...
    public float[] loadAndReadWithOffset(String path, int sampleRate, int readDurationInSeconds, int offsetDuration)
        ...
        float[] magValue = magValueArray[0]; // yangqin
     ...
     * This function loads the audio file, reads its Numeric Magnitude Feature
     * values and then takes the mean of amplitude values across all the channels
     * and convert the signal to mono mode
     *
     * @param path
     * @param sampleRate
     * @param readDurationInSeconds
     ...
     * @throws FileFormatNotSupportedException
     */
    public float[][] loadAndReadStereoWithOffset(String path, int sampleRate, int readDurationInSeconds,
                                                 int offsetDuration) throws IOException, WavFileException, FileFormatNotSupportedException {
     ...
     * @throws FileFormatNotSupportedException
     */
    public ArrayList<Float> loadAndReadAsListWithOffset(String path, int sampleRate, int readDurationInSeconds,
                                                        int offsetDuration) throws IOException, WavFileException, FileFormatNotSupportedException {
