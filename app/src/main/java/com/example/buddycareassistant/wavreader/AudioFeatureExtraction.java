...
public class AudioFeatureExtraction {
    ...
    /**
     * yangqin: This function converts melspec into logmelspec
     *
     * @param y
     * @return
     */
    public double[][] logmelspec(float[] y) {
        final double[][] specTroGram = powerToDb(melSpectrogram(y));
    ...
    /**
     * This function generates mel spectrogram values with extracted STFT features
     * as complex values
     *
     * @param y
     * @return
    ...
    public float[][] melSpectrogramWithComplexValueProcessing(float[] y) {
        ...
        for (int i = 0; i < spectro.length; i++) {
            for (int j = 0; j < spectro[0].length; j++) {
                ...
                double spectroDblVal = Math
                        .sqrt((Math.pow(complexVal.getReal(), 2) + Math.pow(complexVal.getImaginary(), 2)));
    ...
    public Complex[][] extractSTFTFeaturesAsComplexValues(float[] y, boolean paddingFlag) {
        ...
        for (int k = 0; k < frame[0].length; k++) {
            ...
            try {
                ...
                // data will be sinusoidal in nature - so taking the values of 1+n_fft/2 only
                // for processing
                for (int i = 0; i < 1 + n_fft / 2; i++) {
                ...
                for (int k1 = 1 + n_fft / 2; k1 < n_fft; k1++) {
                    cmplxINV1DArr[k1] = new Complex(cmplxINV1DArr[k1 - j_index].getReal(),
                            -1 * cmplxINV1DArr[k1 - j_index].getImaginary());
                    j_index = j_index + 2;
                }
    ...
    public float[] extractInvSTFTFeaturesAsFloatValues(Complex[][] cmplxSTFTValues, boolean paddingFlag) {
        ...
        for (int i = 0; i < cmplxSTFTValues[0].length; i++) {
            ...
            // as the values are inverse in nature - recreating the second half from the
            // first off
            // for n_fft value of 4096 - value at the index of 2049 and 2047 will be same
            // and the sequence will continue for (2050-2046), (2051-2045) etc
            // below loop will recreate those values
            int j_index = 2;
            for (int k = 1 + n_fft / 2; k < n_fft; k++) {
                cmplx1DArr[k] = new Complex(cmplx1DArr[k - j_index].getReal(),
                        -1 * cmplx1DArr[k - j_index].getImaginary());
                j_index = j_index + 2;
            }
    ...
    private double[][] padCenter(double[] fftwin, int size) {
        ...
        // this is a temp way for padding...this code needs to be updated as per
        // pad_center method of istft function
